...
#error "A proper consumer HID report usage range must be selected"
    ...
    // 声明支持的用法范围：0x81（POWER_DOWN）至0x9B（DO_NOT_DISTURB）
    HID_USAGE_MIN8(HID_USAGE_GD_SYSTEM_POWER_DOWN),
    HID_USAGE_MAX8(HID_USAGE_GD_SYSTEM_DO_NOT_DISTURB),
    HID_LOGICAL_MIN8(0x00),
    HID_LOGICAL_MAX8(0x01),
    HID_REPORT_SIZE(0x01),
    HID_REPORT_COUNT(0x1B), // 27 bits
    HID_INPUT(0x02), // 数据、变量、绝对

    // 填充位（补全8位，HID报告需字节对齐）
    HID_REPORT_SIZE(0x01),
    HID_REPORT_COUNT(0x05),
    HID_INPUT(0x03), // 常量（不影响功能）
...
struct zmk_hid_generic_desktop_report_body {
    //uint8_t keys[2]; //16keys
    uint8_t keys[4]; //32keys 0x81 to 0x9B (27 keys)
} __packed;
